...
class FAA():
  ...
  def factor(self,etf, etfs, period):
    ...
    ema = []
    for i in range(len(etf)):
        ema.append(etf[i].resample('M').last())
        ema[i]['ema'] = np.nan
        ema[i]['ema'][period] = etf[i].resample('M').last()['Adj Close'][:period].sum()/period
        for j in range(1,int(len(etf[0].resample('M').last())-period)):
            ema[i]['ema'][j+period] = etf[i].resample('M').last()['Adj Close'][j+period]*0.4 + ema[i]['ema'][j+period-1]*0.6
        ema[i] = ema[i]['ema'].dropna()
